...
const AnonymousStrategy: any = require("passport-anonymous")
...
const PRO_PASSes: string = process.env.PRO_PASS
  ? process.env.PRO_PASS
  : "BITBOX"
// Convert the pro-tier password string into an array split by ':'.
const PRO_PASS: string[] = PRO_PASSes.split(":")
    ...
    */
    ...
    passport.use(
      new BasicStrategy({ passReqToCallback: true }, function(
        ...
        if (username === "BITBOX") {
          for (let i: number = 0; i < PRO_PASS.length; i++) {
            const thisPass: string = PRO_PASS[i]
